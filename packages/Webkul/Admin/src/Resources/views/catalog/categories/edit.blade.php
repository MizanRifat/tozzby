...
{{--  --}}
                            <div class="control-group" :class="[errors.has('show') ? 'has-error' : '']">
                                <label for="status" class="required">Show in Frontend</label>
                                <select class="control" v-validate="'required'" id="show" name="show_" >
                                    <option value="1" {{ $category->show_ ? 'selected' : '' }}>
                                        {{ __('admin::app.catalog.categories.yes') }}
                                    </option>
                                    <option value="0" {{ $category->show_ ? '' : 'selected' }}>
                                        {{ __('admin::app.catalog.categories.no') }}
                                    </option>
                                </select>
                                <span class="control-error" v-if="errors.has('show')">@{{ errors.first('show') }}</span>
                            </div>
                                ...
                                <select class="control" name="attributes[]" v-validate="'required'" data-vv-as="&quot;{{ __('admin::app.catalog.categories.attributes') }}&quot;" multiple>
                                    ...
                                    @foreach ($attributes as $attribute)
                                        <option value="{{ $attribute->id }}" {{ in_array($attribute->id, $selectedaAtributes) ? 'selected' : ''}}>
                                            {{ $attribute->name ? $attribute->admin_name : $attribute->name }}
                                        </option>
                                    @endforeach
